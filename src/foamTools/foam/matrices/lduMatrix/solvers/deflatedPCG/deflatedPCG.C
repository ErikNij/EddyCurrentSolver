...
namespace Foam
{
    defineTypeNameAndDebug(deflatedPCG, 0);

    lduSolver::addsymMatrixConstructorToTable<deflatedPCG>
        adddeflatedPCGSymMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::deflatedPCG::deflatedPCG
(
...
Foam::lduSolverPerformance Foam::deflatedPCG::solve
(
    scalarField& x,
    const scalarField& b,
...
) const
{
    ...
    // Deflation
    wA -= gAverage(matrix_.diag()) / returnReduce(matrix_.diag().size(), sumOp<label>()) * gSum(x);
    ...
    if (!stop(solverPerf))
    {
        ...
        do
        {
            ...
            // Deflation
            wA -= gAverage(matrix_.diag()) / returnReduce(matrix_.diag().size(), sumOp<label>()) * gSum(pA);
