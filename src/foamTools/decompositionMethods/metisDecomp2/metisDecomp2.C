...
namespace Foam
{
    defineTypeNameAndDebug(metisDecomp2, 0);
...
Foam::label Foam::metisDecomp2::decompose
(
...
)
{
    ...
    if (decompositionDict_.found("metisCoeffs"))
    {
        ...
        if (metisCoeffs.readIfPresent("method", method))
        {
            ...
            {
                FatalErrorIn("metisDecomp2::decompose()")
                    << "Method " << method
            ...
            Info<< "metisDecomp2 : Using Metis method     " << method
                << nl << endl;
        }
        ...
        if (metisCoeffs.readIfPresent("options", mOptions))
        {
            ...
            Info<< "metisDecomp2 : Using Metis options     " << mOptions
                << nl << endl;
        }
        ...
        if (metisCoeffs.readIfPresent("processorWeights", processorWeights))
        {
            ...
            {
                FatalErrorIn("metisDecomp2::decompose(const pointField&)")
        ...
        {
            Info<< "metisDecomp2 : Using cell-based weights." << endl;
...
Foam::metisDecomp2::metisDecomp2
(
...
Foam::labelList Foam::metisDecomp2::decompose
(
...
)
{
    if (points.size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "metisDecomp2::decompose(const pointField&,const scalarField&)"
